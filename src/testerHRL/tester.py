...
class TesterHRL:
    ...
    def run_test(self, step, sess, q, test_function, *test_args):
        t_init = time.time()
        ...
        for task_specification in self.get_task_specifications():
            ...
            reward, q = test_function(sess, q, reward_machines, task_params, task_rm_id, self.learning_params, self.testing_params, *test_args)
    ...
    def plot_performance(self,steps,p25,p50,p75):
        ...
        fig, ax = plt.subplots()
        ax.plot(steps,p25, color='blue')
        ax.plot(steps,p50, color='black')
        ax.plot(steps,p75, color='red')
        ax.grid()
